    ...
    async function fetchUserDeposits() {
      try {
        ...
        const { data, error } = await supabase
          .from('deposits')
          .select('id, amount, transaction_hash, status, created_at')
          .eq('user_id', user.id)
        ...
        console.error("Error fetching deposit log stacks:", err);
      } indexing: {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs font-mono text-slate-500 uppercase tracking-widest">Querying Transaction Block...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-slate-800 flex justify-between items-center">
          <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">Clearing Ledger Queue</h3>
        ...
        {deposits.length === 0 ? (
          <div className="p-12 text-center text-sm text-slate-500 font-mono">
            No capital tracking blocks linked to this profile node.
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-950/40 text-2xs uppercase tracking-wider text-slate-400 font-bold border-b border-slate-800">
                  ...
                  <th className="p-4">Fund Volume</th>
                  <th className="p-4">Transaction Sequence Reference</th>
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono text-xs text-slate-300">
                {deposits.map((dep) => (
                  <tr key={dep.id} className="hover:bg-slate-800/20 transition-colors">
                    <td className="p-4 pl-6 font-bold text-slate-500 truncate max-w-[100px]">{dep.id}</td>
                    <td className="p-4 font-bold text-emerald-400">${dep.amount.toLocaleString()}</td>
                    <td className="p-4 text-slate-400 truncate max-w-[180px] text-2xs">{dep.transaction_hash}</td>
