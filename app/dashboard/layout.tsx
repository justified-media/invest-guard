    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex font-sans overflow-hidden h-screen">
      ...
      <aside className="w-64 bg-slate-900 border-r border-slate-800 hidden md:flex flex-col justify-between p-6 shrink-0 h-full">
        <div className="space-y-8">
          ...
          <nav className="space-y-2">
            <Link href="/dashboard" className="flex items-center gap-3 px-4 py-2.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl font-medium transition-all">
              📊 Overview
            </Link>
            <Link href="/dashboard/invest" className="flex items-center gap-3 px-4 py-2.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl font-medium transition-all">
              ⚡ Invest Tiers
            </Link>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-y-auto">
        ...
        <div className="md:hidden bg-slate-900 border-b border-slate-800 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
          ...
          {/* Your Hamburger Menu Trigger Component */}
          <MobileNav />
        </div>
